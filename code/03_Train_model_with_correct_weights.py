...
import shap
###########################################################################################################################################
# This is a corrected version for model training which ensures that ALL models are trained with balanced weights (rf, gbt, lr added)
###########################################################################################################################################
...
def main(path, input, model, cluster,tc):
    ...
    if cv:
        if cluster:
            ...
            utils.tanimoto_cv_with_class_weights(absolute_path, path, prefix, X_train_cluster, y_train_cluster, groups_train_cluster,tc )

        else:
            utils.model_cv_with_weights(absolute_path, path, prefix, X_train, y_train)
